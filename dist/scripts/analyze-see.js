...
const REJECTED_FILE = node_path_1.default.resolve('./validations/see-rejected.json');
function analyzeSEERejections(filePath) {
    ...
    console.log(`Threshold: ${parser_1.SIM_THRESHOLD.toString()}`);
    console.log();
    ...
    for (const bin of Object.keys(bins).sort((a, b) => Number.parseFloat(a) - Number.parseFloat(b))) {
    ...
    console.log();
    // Borderline examples
    const borderline = data.filter((d) => d.sim >= parser_1.SIM_THRESHOLD - 0.05 && d.sim < parser_1.SIM_THRESHOLD);
    if (borderline.length > 0) {
        console.log('🔍 Borderline rejections (within 0.05 of threshold):');
        for (const entry of borderline.slice(0, 20)) {
            console.log(`  ${entry.from} → ${entry.to}  (sim=${entry.sim.toFixed(3)})`);
        }
